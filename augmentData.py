...
def count_multi_labels(dataset_path: str):
    ...
    # Use itertools.chain.from_iterable to flatten the list of lists into a single list of all labels
    ...
    # Use Counter to get the frequency of each unique label
    flavor_counts = Counter(all_flavors)
    filling_counts = Counter(all_fillings)
    icing_counts = Counter(all_icings)

    # Calculate total examples to determine the overall prevalence percentage later
